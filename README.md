...
| Ambiente | Abordagem | Arquivo |
|----------|-----------|---------|
| **Produção** | Plugins na imagem (imutável) | `Dockerfile.plugins` |
| **Desenvolvimento** | Plugins em volume (editável) | `docker-compose.dev.yml` |

### Adicionando Plugins

1. **Edite o manifesto** `plugins/plugins.json`:
...
```json
{
    ...
    "type": "mod",
    "source": "github",
    "repo": "usuario/moodle-mod_seuplugin",
    "version": "v1.0.0",
    "enabled": true
}
```

2. **Tipos de plugins suportados**: mod, block, theme, auth, enrol, local, report, tool, format, qtype, filter, repository, availability, etc.

3. **Fontes suportadas**:
   - `github`: Download de releases/tags do GitHub
   - `git`: Clone direto do repositório
   - `url`: Download de URL direta (ZIP)
...
Para desenvolver plugins localmente:

```bash
# Inicia ambiente de dev
docker-compose -f docker-compose.yml -f docker-compose.dev.yml up -d

# Edite docker-compose.dev.yml para montar seu plugin:
# volumes:
...
Para plugins que não podem ser baixados automaticamente, coloque-os em `plugins/local/`:
...
E descomente a linha no `Dockerfile.plugins`:
```dockerfile
...
```bash
# Lista plugins configurados
make list-plugins

# Verifica plugins no container
make verify-plugins

# Executa upgrade após adicionar plugin
make upgrade
```
